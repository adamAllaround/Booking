...
public class LoggingAspect {
    ...
    @Before("execution(public * com.allaroundjava.booking.domain.domain.ports.*Service.*(..))")
    public void beforeServicePublicMethod(JoinPoint joinPoint) {
        log.info("Calling {}", joinPoint.toShortString());
    }

    @AfterReturning(pointcut = "execution(public * com.allaroundjava.booking.domain.domain.ports.*Service.*(..))",
    returning = "returnValue")
    public void afterServicePublicMethod(Object returnValue) {
        log.info("Returning {}", returnValue.toString());
    }

    @AfterThrowing(pointcut = "execution(public * com.allaroundjava.booking.domain.domain.ports.*Service.*(..))",
            throwing = "exception")
